...
\begin{tikzpicture} [start chain=going below]
	...
	\node[block, below of = start, join, yshift=0.4cm] (init) {Q := дисциплина обслуживания};
	\node[subroutine, below of = init, join, yshift=0.4cm] (class) {cl := find\_min(Q)};
	\node[subroutine, text width = 3cm, below of = class, join, yshift=.4cm] (deq) {pkt := dequeue(cl)};
	\node[decision, text width = 1.5cm, join, yshift=1.4cm,below of = deq] (qempt) {cl.queue пуста};
	\node[block, below of = qempt] (updsn) {cl.sn := 0};
	\node[decision, text width = 1.5cm, join, yshift=1.0cm,  below of=updsn] (isallact) {Все классы активны};
	\node[block, below of = isallact, yshift = -0.3cm] (updcyc) {Q.sn := pkt.sn};
	\node[block, below of = updcyc, join] (stop) {Вернуть pkt};
	\node[cloud, below of = stop, yshift=2.6cm, join] (end) {Конец};
...
%	\path[line] (chkcl) -- node[right] {Да} (deq);


%	\node[block, yshift=0.5cm] (update) {cl.sn := cl.sn + vl};
%	\node[block, below of = update, join] (updatepkt) {pkt.sn := cl.sn};
%	\node[cloud, below of = updatepkt, yshift=2.5cm, join] (stop) {Stop};
